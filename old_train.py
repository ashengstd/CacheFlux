...
def sample_files(
    ...
    sampled_files = []
    # 抽取 50% 概率选择 228-288 范围内的文件
    while len(sampled_files) < max_files and (peek_period or normal_period):
        if random.random() < 0.5 and peek_period:  # 50%概率从 228-288 中选择
...
def train_droo_net_pulp_pipline(
    ...
    droo_data: dict = sio.loadmat(droo_input_path.joinpath(f"{timepoint + 1}.mat"))
    ...
    droo_output: np.ndarray = np.array(
        MemoryDNN_Net.decode(timepoint_data, N, decoder_mode)
    )
    # 计算需求矩阵
...
if __name__ == "__main__":
    ...
    with Progress(console=console) as progress:
            ...
            for daily_dir in INPUT_DATA_PATH.joinpath(month).iterdir():
                ...
                for timepoint_file in sampled_files:
                    ...
                    if solutions_nums == 0:
                        ...
                        np.save(
                            best_solution_save_path.joinpath(f"{timepoint + 1}.npy"),
                            best_solution,
                        )

                    if epoch % 5 == 0:
                        MemoryDNN_Net.save_state_dict(
                            MODEL_SAVE_PATH.joinpath("latest.pth")
                        )
                # 保存结果
        ...
        MemoryDNN_Net.save_state_dict(MODEL_SAVE_PATH.joinpath("final.pth"))
        console.print("训练完成!")
